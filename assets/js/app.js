...
var THEME_PALETTES = {
  ...
  ocean: {
    bgGradient: ["#040d14", "#0a1a2e", "#061020"],
    waveHues: [185, 195, 205],
    blobHueBase: 190,
    accentColor: "#22d3ee"
  },
  cherry: {
    bgGradient: ["#0f0408", "#1e0a14", "#140810"],
    waveHues: [340, 350, 355],
    blobHueBase: 345,
    accentColor: "#f43f5e"
  },
  amber: {
    bgGradient: ["#0f0a04", "#1e1408", "#14100a"],
    waveHues: [30, 38, 42],
    blobHueBase: 35,
    accentColor: "#f59e0b"
  },
  rose: {
    bgGradient: ["#0f040a", "#1e0a14", "#140810"],
    waveHues: [330, 340, 345],
    blobHueBase: 335,
    accentColor: "#fb7185"
  },
  neon: {
    bgGradient: ["#020a04", "#04140a", "#030f08"],
    waveHues: [130, 140, 145],
    blobHueBase: 138,
    accentColor: "#4ade80"
  },
  midnight: {
    bgGradient: ["#04080f", "#0a1428", "#060e1c"],
    waveHues: [215, 225, 235],
    blobHueBase: 225,
    accentColor: "#6366f1"
  },
  forest: {
    bgGradient: ["#040f08", "#0a1e10", "#06140c"],
    waveHues: [140, 150, 155],
    blobHueBase: 148,
    accentColor: "#34d399"
  },
  lavender: {
    bgGradient: ["#0a060f", "#140a1e", "#0f0814"],
    waveHues: [260, 270, 275],
    blobHueBase: 268,
    accentColor: "#c084fc"
  },
  light: {
...
/* v88: Theme registry — names, colors, and order for the theme picker */
var THEMES = [
  { id: "twilight",  name: "Twilight",  color: "#7fd1a4" },
...
function initAurora() {
  ...
  function regenerateStars(){
    ...
    for(var i=0;i<starCount;i++){
      stars.push({
        ...
        /* v87: Extra glow radius for pulsing effect */
        glowR: 0,
        glowPhase: Math.random()*Math.PI*2,
        glowSpeed: 0.003+Math.random()*0.006
      });
    }
  }
  function resize(){
  ...
  var _auroraTargetFPS = N96.isPlaying ? 30 : 20;
  function draw(timestamp){
    /* v87: Skip rendering when tab is hidden — saves CPU */
    if (!isTabVisible) {
      _auroraRAF = window.requestAnimationFrame(draw);
      _auroraRunning = true;
      return;
    }
    /* v87: Frame rate throttling — reduce CPU usage */
    if (timestamp - _lastAuroraTime < (1000 / _auroraTargetFPS)) {
      _auroraRAF = window.requestAnimationFrame(draw);
      _auroraRunning = true;
      return;
    }
    _lastAuroraTime = timestamp;
    ...
    _starGlowBoost += ((isPlaying ? 0.35 : 0) - _starGlowBoost) * 0.02;
    for(i=0;i<stars.length;i++){
      var s=stars[i],twk=Math.sin(frame*s.ts+s.p2)*0.5+0.5;
      var baseAlpha = s.a + twk*0.25 + _starGlowBoost;
      /* v87: Extra glow pulse when music is playing */
      var glowPulse = isPlaying ? (Math.sin(frame*s.glowSpeed+s.glowPhase)*0.5+0.5) : 0;
      var glowSize = s.r * (1.5 + glowPulse * 2 + _starGlowBoost);
      /* Draw glow halo when playing */
      if (isPlaying && _starGlowBoost > 0.05) {
        ctx.beginPath();
        ctx.arc(s.x, s.y, glowSize, 0, Math.PI*2);
        ctx.fillStyle = "rgba(200,230,255," + (baseAlpha * 0.15 * _starGlowBoost).toFixed(3) + ")";
        ctx.fill();
      }
      /* Draw star core */
      ctx.beginPath();ctx.arc(s.x,s.y,s.r*(0.75+twk*0.3),0,Math.PI*2);
      ctx.fillStyle="rgba(215,228,245,"+(baseAlpha).toFixed(3)+")";
      ctx.fill();
    }
...
function drawSpectrum() { if(!analyser||!vizCanvas)return;var ctx=vizCanvas.getContext("2d");
  /* v87: Throttle spectrum FPS for performance */
  var _lastSpecTime = 0;
  function render(timestamp){
    ...
    /* v87: Throttle to 30fps */
    if (timestamp - _lastSpecTime < 33) {
      _spectrumRAF = requestAnimationFrame(render);
      _spectrumRunning = true;
      return;
    }
    _lastSpecTime = timestamp;
...
function toggleTheme() { var h=document.documentElement,c=h.getAttribute("data-theme"),n=c==="twilight"?"dark":c==="dark"?"light":"twilight";setTheme(n); }

/* v88: Set a specific theme by ID */
function setTheme(themeId) {
  var h = document.documentElement;
  h.setAttribute("data-theme", themeId);
  localStorage.setItem("n96-theme", themeId);
  updateAuroraTheme();
  renderThemePicker();
}

/* v88: Theme picker dropdown */
...
function toggleThemePicker() {
  ...
  if (dropdown) {
    dropdown.classList.toggle('open', _themePickerOpen);
  }
  if (_themePickerOpen) {
    renderThemePicker();
  }
...
function renderThemePicker() {
  var dropdown = document.getElementById('theme-picker-dropdown');
  if (!dropdown) return;
  var currentTheme = document.documentElement.getAttribute("data-theme") || "twilight";
  var titleEl = dropdown.querySelector('.theme-picker-title');
  var html = '';
  for (var i = 0; i < THEMES.length; i++) {
    var t = THEMES[i];
    var isActive = t.id === currentTheme;
    html += '<button class="theme-picker-item' + (isActive ? ' active' : '') + '" data-theme-id="' + t.id + '" onclick="setTheme(\'' + t.id + '\')">';
    html += '<span class="theme-dot" style="background:' + t.color + '"></span>';
    html += '<span class="theme-name">' + t.name + '</span>';
    html += '<svg class="theme-check" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>';
    html += '</button>';
  }
  /* Keep the title and replace the items */
  if (titleEl) {
    /* Remove old items */
    var oldItems = dropdown.querySelectorAll('.theme-picker-item');
    for (var j = 0; j < oldItems.length; j++) oldItems[j].remove();
    titleEl.insertAdjacentHTML('afterend', html);
  }
}

/* v88: Close theme picker when clicking outside */
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  applySidebarCollapseState(); restoreSidebarCollapseState(); renderCollectionsSidebar(); renderYtMixesSidebar(); renderYtPlaylistsSidebar(); initSpotify();
  /* v88: Initialize theme picker */
  renderThemePicker();
...
function createYTPlayer() {
  ...
  ytPlayer = new YT.Player("yt-player", {
    ...
    playerVars: {
      autoplay: 0,
      controls: 0,    /* v85: Hide YouTube's own controls — we have our own playback bar */
      rel: 0,
      modestbranding: 1,
      playsinline: 1,
      iv_load_policy: 3,
      disablekb: 0,   /* v85: Keep keyboard controls so user can still use space/arrow keys */
      fs: 0,           /* v85: Hide fullscreen button inside YouTube player */
      showinfo: 0      /* v85: Deprecated but may still help hide title bar in some cases */
    },
    events: {
      onReady: onYTReady,
...
function onYTReady() {
  ...
  /* v86: Auto-hide YouTube control overlays after 3 seconds.
     The overlays cover YouTube's title bar and "More videos" bar.
     They auto-hide after 3 seconds of no mouse movement over the video,
     and reappear on mouse move. This mimics YouTube's native auto-hide. */
  /* v87: Removed overlay auto-hide — no more black overlays.
     The glowing border on the wrapper handles the visual treatment. */
}

/* v87: Removed setupYtOverlayAutoHide — no more black overlays.
   The glowing border replaces the old opaque overlays. */
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
/* v86: SEPARATE localStorage keys for section collapse vs sidebar collapse.
   Previously both used "n96-sidebar-collapsed" which caused a type collision:
   toggleSidebarCollapse saved 'true'/'false' (string), but getCollapsedSections
   expected an array. JSON.parse('true') returns boolean true, and true.indexOf
   is not a function. */
var SIDEBAR_SECTIONS_KEY = "n96-sidebar-sections";  /* array of collapsed section IDs */
var SIDEBAR_COLLAPSED_KEY = "n96-sidebar-collapsed"; /* 'true'/'false' for whole sidebar */
...
var ACCORDION_SECTIONS = ["playlists-section", "yt-mixes-section", "yt-playlists-section", "spotify-section"];
/* Independent section — toggles on its own, not part of the accordion. */
var INDEPENDENT_SECTIONS = ["collections-section"];

function getCollapsedSections() {
  try {
    var raw = localStorage.getItem(SIDEBAR_SECTIONS_KEY);
    if (!raw) return [];
    var parsed = JSON.parse(raw);
    /* v86: Guard against corrupt data — must be an array */
...
function setCollapsedSections(arr) {
  try { localStorage.setItem(SIDEBAR_SECTIONS_KEY, JSON.stringify(arr)); } catch(_) {}
}
...
function expandSidebarForSource(sourceType) {
  ...
  if (sourceType === 'youtube') {
    ...
    sectionId = 'yt-playlists-section';
...
/* ═══════════════════════════════════════════════════════════════
   v85: Sidebar Collapse — hides all sidebar content except logo + slogan
   Click again to restore. Saves state to localStorage.
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   v85: SAVED YOUTUBE PLAYLISTS — localStorage-backed collection
   Stored separately from mixes (which are individual videos).
   Playlists are full YouTube playlists with multiple videos.
   Storage key: "n96-yt-playlists"
   ═══════════════════════════════════════════════════════════════ */
var YT_PLAYLISTS_KEY = "n96-yt-playlists";

function loadSavedPlaylists() {
  try {
    var raw = localStorage.getItem(YT_PLAYLISTS_KEY);
    return raw ? JSON.parse(raw) : [];
  } catch(_) { return []; }
}

function saveSavedPlaylists(playlists) {
  try { localStorage.setItem(YT_PLAYLISTS_KEY, JSON.stringify(playlists)); } catch(_) {}
}

function renderYtPlaylistsSidebar() {
  var list = document.getElementById('yt-playlists-list');
  if (!list) return;
  var playlists = loadSavedPlaylists();
  // Preserve the "Load Playlist" button
  var addBtn = list.querySelector('.playlist-add-btn');
  list.innerHTML = '';
  if (addBtn) list.appendChild(addBtn);

  for (var i = 0; i < playlists.length; i++) {
    var p = playlists[i];
    var isActive = (ytPlaylistState.active && ytPlaylistState.url === p.url) ? 'active' : '';
    /* v87: Use <div> instead of <button> — buttons don't support drag-and-drop
       properly in many browsers. The div is styled identically. */
    var item = document.createElement('div');
    item.className = 'playlist-item ' + isActive;
    item.setAttribute('data-url', p.url);
    item.setAttribute('role', 'button');
    item.setAttribute('tabindex', '0');
    item.innerHTML =
      '<span class="playlist-icon"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"></line><line x1="8" y1="12" x2="21" y2="12"></line><line x1="8" y1="18" x2="21" y2="18"></line><line x1="3" y1="6" x2="3.01" y2="6"></line><line x1="3" y1="12" x2="3.01" y2="12"></line><line x1="3" y1="18" x2="3.01" y2="18"></line></svg></span>' +
      '<span class="playlist-name">' + esc(p.name || 'Untitled Playlist') + '</span>' +
      '<span class="playlist-count">' + (p.videoCount || '?') + '</span>' +
      '<span class="playlist-delete" title="Remove this playlist"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></span>';
    (function(playlist) {
      item.addEventListener('click', function(e) {
        if (e.target.closest('.playlist-delete')) {
          e.stopPropagation();
          deleteSavedPlaylist(playlist.url);
          return;
        }
        /* v87: One-click play — fetch and play directly in the background */
        playSavedPlaylistDirectly(playlist.url, playlist.name);
      });
      /* v87: Keyboard support for the div-based playlist item */
    ...
    /* v87: Make playlist items draggable for collection drop */
    item.setAttribute('draggable', 'true');
    item.addEventListener('dragstart', handleMixDragStart);
    item.addEventListener('dragend', handleMixDragEnd);
    list.appendChild(item);
  }
}

function savePlaylistToSidebar() {
  if (!ytPlaylistState.videos.length) {
    showToast('No playlist loaded to save.', 'error');
    return;
  }
  var nameInput = document.getElementById('yt-playlist-name-input');
  var customName = nameInput ? (nameInput.value || '').trim() : '';
  var playlistName = customName || ytPlaylistState.title || 'YouTube Playlist';

  var playlists = loadSavedPlaylists();
  // Check if already saved
  for (var i = 0; i < playlists.length; i++) {
    if (playlists[i].url === ytPlaylistState.url) {
      playlists[i].name = playlistName;
      playlists[i].videoCount = ytPlaylistState.videos.length;
      saveSavedPlaylists(playlists);
      renderYtPlaylistsSidebar();
      showToast('Playlist updated: ' + playlistName, 'info');
      return;
    }
  }
  // Add new
  playlists.push({
    name: playlistName,
    url: ytPlaylistState.url,
    videoCount: ytPlaylistState.videos.length,
    addedAt: Date.now()
  });
  saveSavedPlaylists(playlists);
  renderYtPlaylistsSidebar();
  showToast('Playlist saved: ' + playlistName, 'info');
}

function deleteSavedPlaylist(url) {
  var playlists = loadSavedPlaylists().filter(function(p) { return p.url !== url; });
  saveSavedPlaylists(playlists);
  renderYtPlaylistsSidebar();
  saveStateNow();
}

function loadSavedPlaylistByUrl(url) {
  // Legacy: still used by collection items — now also plays directly
  playSavedPlaylistDirectly(url);
}

/* v87: One-click play — fetch playlist in background and auto-play.
   No modal needed — just fetch, set state, and start playing. */
async function playSavedPlaylistDirectly(url, name) {
  if (!url) return;
  ...
  try {
    ...
    if (!res.ok || data.error) {
      showToast('Failed to load playlist: ' + (data.error || 'Unknown error'), 'error');
      return;
    }

    /* Look up the saved playlist name if not provided */
    if (!name) {
      var saved = loadSavedPlaylists();
      for (var i = 0; i < saved.length; i++) {
        if (saved[i].url === url) { name = saved[i].name; break; }
      }
    }

    /* Set playlist state */
    ytPlaylistState.title = name || data.playlistTitle || 'YouTube Playlist';
    ytPlaylistState.videos = data.videos || [];
    ...
    showToast('Failed to fetch playlist: ' + e.message, 'error');
  }
}
...
function renderYtMixesSidebar() {
  ...
  // v85: Preserve only the .mix-add-btn (Add Mix with link — Load Playlist is now in its own section)
  ...
  for (var i = 0; i < mixes.length; i++) {
    ...
    /* v89: Use <div> instead of <button> — buttons don't support drag-and-drop
       properly in many browsers. The div is styled identically. */
    var item = document.createElement("div");
    ...
    (function(mix){
      ...
      /* v89: Keyboard support for the div-based mix item */
      item.addEventListener("keydown", function(e) {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          playYouTube({
...
function renderSpotifySidebar() {
  ...
  for (var i = 0; i < playlists.length; i++) {
    ...
    (function(pl){
      ...
      /* v89: Keyboard support for the div-based spotify item */
      item.addEventListener("keydown", function(e) {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          playSpotifyPlaylist(pl);
...
async function fetchYouTubePlaylist() {
  ...
  try {
    ...
    // Store in state — v85: use custom name if provided
    var nameInput = document.getElementById('yt-playlist-name-input');
    var customName = nameInput ? (nameInput.value || '').trim() : '';
    ytPlaylistState.title = customName || data.playlistTitle || 'YouTube Playlist';
    ytPlaylistState.videos = data.videos || [];
    ytPlaylistState.url = url;
    ...
    (function(col) {
      ...
      (function(collectionId) {
        colEl.addEventListener("dragenter", function(e) {
          /* v89: Prevent default to allow drop on this element */
          e.preventDefault();
      ...
      for (var i = 0; i < col.items.length; i++) {
        (function(item, idx) {
          ...
          itemEl.className = "collection-item " + (item.type === "youtube" ? "item-yt" : item.type === "yt-playlist" ? "item-yt-playlist" : "item-spotify");
          ...
          if (N96.nowPlaying) {
            if (item.type === "youtube" && N96.nowPlaying.isYouTube && !N96.nowPlaying.isSpotify && N96.nowPlaying.videoId === item.id) {
              isActiveItem = true;
            } else if (item.type === "yt-playlist" && ytPlaylistState.active && ytPlaylistState.url === item.id) {
              isActiveItem = true;
...
function playCollectionItem(item) {
  ...
  } else if (item.type === "yt-playlist") {
    /* v86: Saved YouTube playlist — load and play by URL */
    loadSavedPlaylistByUrl(item.id);
...
function handleMixDragStart(e) {
  ...
  var plItem = e.target.closest(".playlist-item");  /* v86: saved YouTube playlists */
  ...
  if (mixItem) {
    ...
    /* v86: Saved YouTube playlists — type is 'yt-playlist', id is the URL */
    type = "yt-playlist";
    id = plItem.getAttribute("data-url") || "";
    title = plItem.querySelector(".playlist-name") ? plItem.querySelector(".playlist-name").textContent : "";
  }

  if (!id) {
    /* v89: If we can't determine the dragged item, abort the drag */
    e.preventDefault();
    return;
  }

  var payload = JSON.stringify({ type: type, id: id, title: title });
  console.log("[N96] DragStart — type:", type, "id:", id, "title:", title);
  e.dataTransfer.setData("text/plain", payload);
  /* v89: Also set as 'application/json' for better cross-browser compatibility */
  try { e.dataTransfer.setData("application/json", payload); } catch(_) {}
  e.dataTransfer.effectAllowed = "copy";

  /* v89: Add a visual 'dragging' class to the source element */
  var sourceEl = mixItem || spItem || plItem;
  if (sourceEl) {
    sourceEl.classList.add("dragging");
    /* Remove the class after a short delay or on dragend */
    setTimeout(function() {
...
function handleMixDragEnd(e) {
  /* v89: Remove the dragging class from source element */
  document.querySelectorAll(".mix-item.dragging, .spotify-item.dragging, .playlist-item.dragging").forEach(function(el) {
...
function handleCollectionDragOver(e, colId) {
  e.preventDefault();
  /* v89: Always use 'copy' drop effect for sidebar items — they're being added, not moved.
     For inter-collection drag, 'move' is correct. Since we can't read data during dragover,
     we default to 'copy' which is the most common case. */
...
function handleCollectionDrop(e, colId) {
  ...
  /* v89: Try both text/plain and application/json for cross-browser compatibility */
  var raw = e.dataTransfer.getData("text/plain");
  if (!raw) {
    try { raw = e.dataTransfer.getData("application/json"); } catch(_) {}
  }
  if (!raw) {
    console.log("[N96] Drop on collection " + colId + " — no drag data received");
    return;
  }

  try {
    var dragData = JSON.parse(raw);
    console.log("[N96] Drop on collection " + colId + " — data:", dragData);
  } catch(_) {
    console.log("[N96] Drop on collection " + colId + " — invalid JSON:", raw);
    return;
  }
...
function restoreCollectionToPlaylists(colId) {
  ...
  var spRestored = 0;
  var spSkipped = 0;
  var ytplRestored = 0;  /* v86: yt-playlist items */
  var ytplSkipped = 0;
  ...
  /* v86: Restore YouTube Playlist items */
  var savedPlaylists = loadSavedPlaylists();
  for (var i = 0; i < col.items.length; i++) {
    var item = col.items[i];
    if (item.type !== "yt-playlist") continue;
    var exists = false;
    for (var p = 0; p < savedPlaylists.length; p++) {
      if (savedPlaylists[p].url === item.id) { exists = true; break; }
    }
    if (exists) {
      ytplSkipped++;
      continue;
    ...
    savedPlaylists.push({
      name: item.title || "YouTube Playlist",
      url: item.id,
      videoCount: 0,
      addedAt: Date.now() + i
    });
  ...
  if (ytplRestored > 0) saveSavedPlaylists(savedPlaylists);
  ...
  renderYtMixesSidebar();
  renderSpotifySidebar();
  if (ytplRestored > 0) renderYtPlaylistsSidebar();

  /* Build summary toast */
  var parts = [];
  if (ytRestored > 0) parts.push(ytRestored + " YouTube mix" + (ytRestored > 1 ? "es" : ""));
  if (ytplRestored > 0) parts.push(ytplRestored + " YouTube playlist" + (ytplRestored > 1 ? "s" : ""));
  ...
  if (ytplSkipped > 0) skippedParts.push(ytplSkipped + " YT playlist already present");
...
function makeSidebarItemsDraggable() {
  ...
  /* v86: Saved YouTube Playlist items */
...
window.savePlaylistToSidebar = savePlaylistToSidebar;
